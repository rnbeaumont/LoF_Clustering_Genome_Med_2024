#!/usr/bin/env Rscript
# R script for reading exon and variant positions and generating regional plots
library(tidyverse)
library(magrittr)
library(vroom)
library(glue)
...
read_exons<-function(exon_file){	# function to read the positions of the exons
	...
	data<-vroom(exon_file,show_col_types=F) %>% # read in the file
		...
		rename(gene=`Gene name`,transcript=`Transcript stable ID`,exonID=`Exon stable ID`,chr=`Chromosome/scaffold name`,start=`Genomic coding start`,end=`Genomic coding end`,strand=Strand) %>% # rename columns
...
calculate_exon_boundaries<-function(data){	# function to calculate exon boundaries relative to coding sequence
	...
	exon_counts<-data %>% group_by(transcript) %>% 
		slice(which.max(relative_end)) %>% select(transcript,relative_end,strand) %>%
		rename(transcript_length=relative_end) %>% merge(exon_counts,.)
...
calculate_exon_boundaries_single_pseudogene<-function(data){	# function to calculate exon boundaries relative to coding sequence
	...
	exon_counts<-data %>% group_by(gene) %>% 
		slice(which.max(relative_end)) %>% select(gene,relative_end,strand) %>%
		rename(gene_length=relative_end) %>% merge(exon_counts,.)
	list(data, exon_counts)
}

read_masks<-function(annotations,freq_file,exon_positions,exon_counts){	# function to read the annotations of variants and append them to a data frame, calculating their relative positions
	# generate list of files containing variant data
	tmp<-unlist(strsplit(annotations,"\\*"))
	variants <- data.table()
	for(i in c(1:22,"X")){	# read data a chromosome at a time and merge with exon data
		variants_tmp <- vroom(glue(tmp[1],i),col_names=F,show_col_types=F,delim="\t") %>%	# read in the files
			rename("variantID"=X1,gtran=X2,annotation=X3) %>%	# rename
			mutate(gene=unlist(lapply(strsplit(gtran,"_"),"[[",1)),transcript=unlist(lapply(strsplit(gtran,"_"),"[[",2)),position=unlist(lapply(strsplit(variantID,":"),"[[",2))) %>%	# split out position, gene name and transcript ID
			mutate(position=as.numeric(position))	# initialise position column
		exon_positions_tmp <- exon_positions %>% filter(chr==i)
		variants_tmp %<>% merge(exon_positions_tmp) %>% filter(start<position & end>position) %>% mutate(position=position-start+relative_start)	# get the position within the gene
		lof<-c("LoF_HC","LoF_HC_50bp_fail","LoF_HC_50bp_pass","LoF_LC","LoF_LC_50bp_fail","LoF_LC_50bp_pass") # list of annotations for lof indels
		variants_tmp <- exon_counts %>% select(transcript,n,transcript_length,strand) %>% rename(n_exons=n) %>% merge(variants_tmp) %>%
				filter(!(annotation %in% lof & transcript_length==relative_end & relative_end-position<=3)) %>% # filter out lof indels in last codon as there are miscalled stop-retained variants here
		       	mutate(position=position/transcript_length) %>% mutate(position=ifelse(strand==-1,1-position,position)) %>%	# calculate relative position as proportion of coding sequence
				select(variantID,gene,transcript,annotation,position,transcript_length,n_exons,strand)	# get columns we want and combine with variant df
		# Read in variant allele frequencies and append them to the data frame
		frq_tmp <- unlist(strsplit(freq_file,"\\*"))
		data<-read_tsv(glue(frq_tmp[1],i,frq_tmp[2]),col_select=c(uploaded_variation,a1,a2,hom_a1,het,hom_a2),show_col_types = FALSE) %>%	# read in the columns we need
			rename(variantID=uploaded_variation) %>% mutate(mac=2*hom_a1+het,maf=mac/(mac+het+2*hom_a2)) %>%	# rename and calculate mac and maf
			select(variantID,mac,maf)	# drop counts we don't need
		variants <- variants_tmp %>% merge(data) %>% 
			distinct() %>% 	# merge with variant data
			rbind(variants,.)
	}
...
read_masks_single_pseudogene<-function(annotations,freq_file,exon_positions,exon_counts){	# function to read the annotations of variants and append them to a data frame, calculating their relative positions
	tmp<-unlist(strsplit(annotations,"\\*"))
	variants <- data.table()
	consequences<-c("synonymous"=1,
		"missense_cadd24less_variant"=2,
		"missense_cadd25plus_variant"=3,
		"missense_cadd30plus_variant"=4,
		"inframe_indel_cadd24less_variant"=5,
		"inframe_indel_cadd25plus_variant"=6,
		"inframe_indel_cadd30plus_variant"=7,
		"LoF_SNV_50bp_fail"=8,
		"LoF_SNV_50bp_pass"=9,
		"LoF_SNV"=10,
		"LoF_LC_50bp_fail"=11,
	...
	lof<-c("LoF_HC","LoF_HC_50bp_fail","LoF_HC_50bp_pass","LoF_LC","LoF_LC_50bp_fail","LoF_LC_50bp_pass") # list of annotations for lof indels
	for(i in c(1:22,"X")){
		variants_tmp<-vroom(glue(tmp[1],i),col_names=F,show_col_types=F,delim="\t") %>%
			rename("variantID"=X1,gtran=X2,annotation=X3) %>%	# rename
			mutate(gene=unlist(lapply(strsplit(gtran,"_"),"[[",1)),transcript=unlist(lapply(strsplit(gtran,"_"),"[[",2)),position=unlist(lapply(strsplit(variantID,":"),"[[",2))) %>%	# split out position, gene name and transcript ID
			mutate(position=as.numeric(position)) %>%	# initialise position column
			select(-c(gtran,transcript))
		variants_tmp %<>% distinct() # remove duplicate rows
		# select the highest consequence for the variants with different consequences across transcripts
		variants_tmp %<>% mutate(annotation_n=consequences[annotation]) %>% 
			group_by(variantID) %>% 
			filter(annotation_n==max(annotation_n)) %>% 
			select(-annotation_n) %>% ungroup()
		exon_positions_tmp <- exon_positions %>% filter(chr==i)
		variants_tmp %<>% merge(exon_positions_tmp) %>% filter(start<position & end>position) %>% mutate(position=position-start+relative_start)	# get the position within the gene
		variants_tmp <- exon_counts %>% select(gene,n,gene_length,strand) %>% rename(n_exons=n) %>% merge(variants_tmp) %>%
				filter(!(annotation %in% lof & gene_length==relative_end & relative_end-position<=3)) %>% # filter out lof indels in last codon as there are miscalled stop-retained variants here
		       	mutate(position=position/gene_length) %>% mutate(position=ifelse(strand==-1,1-position,position)) %>%	# calculate relative position as proportion of coding sequence
				select(variantID,gene,annotation,position,gene_length,n_exons,strand)	# get columns we want and combine with variant df
		frq_tmp <- unlist(strsplit(freq_file,"\\*"))
		data<-read_tsv(glue(frq_tmp[1],i,frq_tmp[2]),col_select=c(uploaded_variation,a1,a2,hom_a1,het,hom_a2)) %>%	# read in the columns we need
			rename(variantID=uploaded_variation) %>% mutate(mac=2*hom_a1+het,maf=mac/(mac+het+2*hom_a2)) %>%	# rename and calculate mac and maf
			select(variantID,mac,maf)	# drop counts we don't need
		variants <- variants_tmp %>% merge(data) %>% 
			distinct() %>% 
...
exon_counts<-tmp[[2]]
variants<-read_masks(annotations,freq_file,exon_positions,exon_counts)
write.table(variants,file=glue(output_prefix,"_all_variants"),quote=F,row.names=F)
...
exon_positions<-tmp[[1]]
exon_counts<-tmp[[2]]
variants<-read_masks_single_pseudogene(annotations,freq_file,exon_positions,exon_counts)
write.table(variants,file=glue(output_prefix,"_single_pseudo_transcript_all_variants"),quote=F,row.names=F)